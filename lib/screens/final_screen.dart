...
import 'package:flutter/services.dart' show rootBundle; // Para cargar assets (logo)
...
class _FinalScreenState extends State<FinalScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scaleAnimation = CurvedAnimation(parent: _animationController, curve: Curves.elasticOut); // Efecto rebote
    _fadeAnimation = CurvedAnimation(parent: _animationController, curve: const Interval(0.3, 1.0, curve: Curves.easeIn)); // Aparece después
  ...
  Future<void> _generateAndSavePdf(String winnerName) async {
    ...
    pdf.addPage(
      pw.Page(
        ...
        build: (pw.Context context) { // Función que construye el contenido
          // Usar widgets de la librería 'pdf/widgets.dart' (prefijo 'pw')
          return pw.Center(
            child: pw.Padding(
              padding: const pw.EdgeInsets.all(30),
              child: pw.Column(
                mainAxisAlignment: pw.MainAxisAlignment.spaceAround, // Espacio vertical
                crossAxisAlignment: pw.CrossAxisAlignment.center,   // Centrado horizontal
                children: [
                  // Mostrar logo si se cargó, si no un placeholder
                  if(logoImage != null)
                     pw.Container(height: 80, child: pw.Image(logoImage)),
                  if(logoImage == null)
                     pw.Container(height: 80, child: pw.Text('[Logo no disponible]')),
    ...
    try {
      if (kIsWeb) { // Si estamos en la web
          ...
          final anchor = html.AnchorElement(href: 'data:application/pdf;base64,$base64Pdf')
            ..setAttribute("download", "certificado_maestro_${winnerName.replaceAll(' ', '_')}.pdf") // Nombre del archivo
            ..click(); // Simular clic para descargar
          html.document.body?.children.remove(anchor); // Limpiar el link del HTML
          if (mounted) ScaffoldMessenger.of(context).showSnackBar( const SnackBar(content: Text('Descargando PDF...')) );
      } else { // Si estamos en Móvil o Escritorio
          // Obtener directorio de guardado apropiado
          final Directory directory;
          if (Platform.isAndroid) { directory = await getExternalStorageDirectory() ?? await getApplicationDocumentsDirectory(); } // Descargas o Documentos en Android
          else { directory = await getApplicationDocumentsDirectory(); } // Documentos en iOS, macOS, Linux, Windows

          final String filePath = '${directory.path}/certificado_maestro_${winnerName.replaceAll(' ', '_')}.pdf';
          final File file = File(filePath);
          await file.writeAsBytes(await pdf.save()); // Escribir los bytes al archivo

          if (kDebugMode) print("PDF guardado en: $filePath");
          // Mostrar notificación con opción para abrir el archivo
          if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar( SnackBar( content: Text('PDF guardado en ${directory.path}'), duration: Duration(seconds: 5), action: SnackBarAction( label: 'Abrir', onPressed: () async { try { await OpenFilex.open(filePath); } catch (e) { if(kDebugMode) print("Error al abrir PDF: $e"); }} )));
          }
      }
    } catch (e) { // Manejar errores al guardar/descargar
        if (kDebugMode) print("Error al generar/guardar PDF: $e");
        if (mounted) ScaffoldMessenger.of(context).showSnackBar( SnackBar(content: Text("Error al generar o guardar PDF.")) );
    }
  ...
  Widget build(BuildContext context) {
    ...
    final String winnerName = winner?.name ?? 'Campeón Desconocido'; // Nombre por defecto si algo falla
    final String masterTitle = 'Maestro Pokémon\n$winnerName'; // Texto principal

    return Scaffold(
      body: Container(
        // Fondo de victoria
        decoration: BoxDecoration( image: DecorationImage( image: const AssetImage('assets/images/victory_bg.png'), fit: BoxFit.cover,),),
        child: Center(
            ...
            child: Column(
              ...
              children: [
                ...
                ScaleTransition(
                  ...
                  child: const Icon(Icons.emoji_events, size: 150.0, color: Colors.amberAccent, shadows: [ Shadow( color: Colors.black54, blurRadius: 10.0, offset: Offset(4, 4)) ],),
                ),
                const SizedBox(height: 30),
                // Texto "Felicidades" animado
                FadeTransition(
                  opacity: _fadeAnimation,
                  child: Text('¡Felicidades!', style: GoogleFonts.pressStart2p(fontSize: 28, color: Colors.white, shadows: [ const Shadow(color: Colors.black45, blurRadius: 5, offset: Offset(2, 2)) ],), textAlign: TextAlign.center,),
                ),
                const SizedBox(height: 20),
                // Nombre del ganador animado
                FadeTransition(
                  opacity: _fadeAnimation,
                  child: Text( masterTitle, style: GoogleFonts.pressStart2p( fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white, height: 1.3, shadows: [ const Shadow(color: Colors.black54, blurRadius: 6, offset: Offset(3, 3)) ], ), textAlign: TextAlign.center,),
                ),
                ...
                FadeTransition(
                  ...
                  child: ElevatedButton.icon(
                    icon: const Icon(Icons.autorenew), label: const Text('Jugar Nuevo Torneo'),
                ...
                FadeTransition(
                  opacity: _fadeAnimation,
                  child: ElevatedButton.icon(
                    icon: const Icon(Icons.picture_as_pdf_outlined), label: const Text('Guardar Certificado PDF'),
                    ...
                    onPressed: () {
                      ...
                      if (currentWinner != null) { _generateAndSavePdf(currentWinner.name); }
                      else { if(mounted) { ScaffoldMessenger.of(context).showSnackBar( const SnackBar(content: Text("Error: No se pudo determinar el ganador.")) ); } }
                    },
                  ),
                ),
              ],
